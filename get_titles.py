...
for index, row in df.iterrows(): #for each site we have referenced
    ...
    #Here we make sure we get just the title
    for title in all_titles_in_soup:
        #if several line space in the title_tag's text, we assume we have too many tags inside, so we use the one define in the dataframe.
        if '\n\n\n' in title.text:
            print(title.prettify())
            title_text = title.find_all(row['title_text'])
            if title_text == []:
                title_text = title.find_all(row['title_text2'])
    ...
    for tag in all_titles_in_soup:
        ...
        #for cases where the contains to much text
        try:
            tag = tag.split("\n\n\n")[0]
        except TypeError:
            pass
